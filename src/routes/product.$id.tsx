...
import { categoryName, fetchProduct, fetchSimilar, formatNaira } from "@/lib/products";
...
export const Route = createFileRoute("/product/$id")({
  loader: async ({ params }) => {
    ...
    const similar = await fetchSimilar(params.id);
      ...
      <main className="mx-auto max-w-6xl px-5 pt-8 animate-fade-up">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-2">
          ...
          <div className="flex flex-col">
            ...
            <p className="mt-6 max-w-prose text-[15px] leading-relaxed text-muted-foreground">{product.description}</p>
            <div className="mt-8 flex items-center gap-4">
              <div className="inline-flex items-center rounded-full border border-border bg-card">
                <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="grid h-11 w-11 place-items-center rounded-full transition hover:bg-muted" aria-label="Decrease"><Minus className="h-4 w-4" /></button>
                <span className="w-10 text-center font-medium">{qty}</span>
                <button onClick={() => setQty((q) => q + 1)} className="grid h-11 w-11 place-items-center rounded-full transition hover:bg-muted" aria-label="Increase"><Plus className="h-4 w-4" /></button>
              </div>
            ...
            <Link to="/checkout" className="mt-3 text-sm text-muted-foreground underline-offset-4 hover:underline">Go to checkout →</Link>
          </div>
        </div>
