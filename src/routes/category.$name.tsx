...
export const Route = createFileRoute("/category/$name")({
  loader: async ({ params }) => {
    ...
    const items = await fetchProductsByCategory(category.slug as CategorySlug);
      ...
      <main className="mx-auto max-w-6xl px-5 pt-8 pb-20 animate-fade-up">
        ...
        <header className="mt-6 grid gap-8 rounded-3xl border border-border bg-card p-6 sm:p-10 md:grid-cols-[1.2fr_1fr]">
          <div className="flex flex-col justify-center">
            ...
            <h1 className="mt-3 font-display text-4xl leading-tight sm:text-5xl">{category.name}</h1>
            <p className="mt-4 max-w-prose text-[15px] leading-relaxed text-muted-foreground">{category.blurb}</p>
          </div>
          <div className="overflow-hidden rounded-2xl bg-muted">
            <img src={category.image} alt={category.name} className="aspect-[5/4] w-full object-cover" />
          </div>
        </header>
